...
static void http_rest_with_url(void)
{
    ...
    // POST
    const char *post_data = "{\"JCBRO\":\"sachin\"}";
    // esp_http_client_set_url(client, "http://httpbin.org/post");
    esp_http_client_set_url(client, "http://ptsv2.com/t/sachin/post");
    // esp_http_client_set_url(client, "http://192.138.0.100:443/zm/test");
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "HTTP POST Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
                esp_http_client_read(client, local_response_buffer, esp_http_client_get_content_length(client)); //by sachin
